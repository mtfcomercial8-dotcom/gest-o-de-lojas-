...
import { LayoutDashboard, List, PieChart as PieIcon, Plus, Menu } from 'lucide-react';
import { Transaction, InsightState, MonthlyDataPoint, ChartDataPoint } from './types';
...
function App() {
  ...
  const handleAddTransaction = (newTx: Omit<Transaction, 'id'>) => {
    ...
    // Clear old insights when data changes
    if (insightState.content) {
        setInsightState(prev => ({ ...prev, content: null }));
    }
  };
  ...
  const handleGenerateInsights = async () => {
    ...
    try {
      const insight = await generateFinancialInsights(transactions);
      setInsightState({ loading: false, content: insight, error: null });
    } catch (err) {
      setInsightState({ 
        loading: false, 
        content: null, 
        error: "Erro ao gerar insights. Verifique sua chave API ou tente novamente mais tarde." 
      });
    }
  };
      ...
      <aside className="bg-white border-r border-slate-200 md:w-64 flex-shrink-0 flex flex-col fixed md:sticky top-0 h-16 md:h-screen w-full z-20 md:z-auto">
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold tracking-tight text-slate-800">FinDash<span className="text-indigo-600">.ai</span></h1>
          </div>
        ...
        <div className="hidden md:block p-4 border-t border-slate-100">
          <div className="bg-slate-900 rounded-xl p-4 text-white">
            <p className="text-xs font-medium text-slate-400 mb-1">Status da API</p>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
              <span className="text-sm font-semibold">Gemini Conectado</span>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 mt-16 md:mt-0 p-4 md:p-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Bem-vindo de volta ao seu painel financeiro.</p>
          </div>
          <button 
        ...
        {/* Stats Grid - Always visible on dashboard, optional on tx list */}
        {activeTab === 'dashboard' && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <AIAdvisor insightState={insightState} onGenerate={handleGenerateInsights} />
